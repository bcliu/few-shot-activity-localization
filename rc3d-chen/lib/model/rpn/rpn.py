...
class _RPN(nn.Module):
    ...
    def forward(self, base_feat, gt_twins):
        ...
        # print('rpn_conv1 out dim: %s' % str(rpn_conv1.shape))
        rpn_conv2 = F.relu(self.RPN_Conv2(rpn_conv1), inplace=True)
        # print('rpn_conv2 out dim: %s' % str(rpn_conv2.shape))
        rpn_output_pool = self.RPN_output_pool(rpn_conv2) # (1,512,96,1,1)
        # print('rpn_output_pool out dim: %s' % str(rpn_output_pool.shape))

        # get rpn classification score
        rpn_cls_score = self.RPN_cls_score(rpn_output_pool)
        # print('rpn_cls_score dim: %s' % str(rpn_cls_score.shape))
        ...
        if self.training:
            ...
            rpn_data = self.RPN_anchor_target((rpn_cls_score.data, gt_twins))  # List of 4 tensors: 1x1x960x1x1, 1x20x96x1x1, same for #3 and #4

            # compute classification loss
            rpn_cls_score = rpn_cls_score_reshape.permute(0, 2, 3, 4, 1).contiguous().view(batch_size, -1, 2)  # 1x960x2
            self.rpn_label = rpn_data[0].view(batch_size, -1)

            rpn_keep = Variable(self.rpn_label.view(-1).ne(-1).nonzero().view(-1))  # Always a 1d vec of length 64
