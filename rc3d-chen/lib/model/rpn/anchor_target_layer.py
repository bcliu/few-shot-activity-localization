...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        rpn_cls_score = input[0]  # 1x20x96x1x1
        # GT boxes (batch_size, n, 3), each row of gt box contains (x1, x2, label)
        gt_twins = input[1]  # 1x20x3, 20 ground truth labels: [start] [end] [label]
        #im_info = input[2]
        #num_boxes = input[2]
        ...
        # map of shape (..., L, H, W)
        length, height, width = rpn_cls_score.shape[-3:]  # L: 96, H: 1, W: 1
        ...
        # NOTE: each non-zero value in overlaps is calculated by (proposal_end-proposal_start+1)/(gt_end-gt_start+1)
...
#           num_bg = cfg.TRAIN.RPN_BATCHSIZE - sum_fg[i]
        ...
        # TODO: possibly bug? argmax_overlaps are all zeros!
