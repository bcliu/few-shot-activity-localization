...
#def test_net(tdcnn_demo, dataloader, dataloader_fewshot, args):
def test_net(tdcnn_demo, dataloader, args):
    ...
    ###
    # for i, (video_data_fewshot, gt_twins_fewshot, num_gt, video_info) in enumerate(dataloader_fewshot):
    #     video_data_fewshot = video_data_fewshot.cuda()
    #     gt_twins_fewshot = gt_twins_fewshot.cuda()
    #     batch_size = video_data_fewshot.shape[0]
    #     _, _, _, pooled_feat_fewshot = tdcnn_demo(video_data_fewshot, gt_twins_fewshot)
    ###
    ...
    for i, (video_data, gt_twins, num_gt, video_info) in enumerate(dataloader):
        ...
        det_tic = time.time()
        ###
        rois, cls_prob, twin_pred, pooled_feat_test = tdcnn_demo(video_data, gt_twins)
        #print("pooled_feat_test shape", pooled_feat_test.shape)
        #_,_,_, pooled_feat_fewshot = tdcnn_demo(video_data_fewshot, gt_twins_fewshot)
        _, _, _, pooled_feat_fewshot = tdcnn_demo(video_data, gt_twins) # for test
        cos_similarity = torch.nn.functional.cosine_similarity(pooled_feat_test, pooled_feat_fewshot, dim=1)
...
if __name__ == '__main__':
    ...
    if args.dataset == "thumos14":
        ...
        args.imdb_fewshot_name = "fewshot_data_25fps.pkl"
        args.num_classes = 21
    ...
    ###
    # roidb_path_fewshot = args.roidb_dir + "/" + args.dataset + "/" + args.imdb_fewshot_name
    # roidb_fewshot = get_roidb(roidb_path_fewshot)
    # dataset_fewshot = roibatchLoader(roidb_fewshot, phase='fewshot')
    # dataloader_fewshot = torch.utils.data.DataLoader(dataset_fewshot, batch_size=args.batch_size,
    #                          num_workers=args.num_workers, shuffle=False)
    #
    # num_videos_fewshot = len(dataset_fewshot)
    # args.num_videos_fewshot = num_videos_fewshot
    # print('{:d} roidb fewshot entries'.format(num_videos_fewshot))
    ###
...
#        tdcnn_demo = nn.parallel.DataParallel(tdcnn_demo, device_ids = args.gpus)
    ...
    #test_net(tdcnn_demo, dataloader, dataloader_fewshot, args)
