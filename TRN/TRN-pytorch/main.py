...
def main():
    ...
    categories, args.train_list, args.val_list, args.root_path, prefix = datasets_video.return_dataset('thumos', args.modality)
    ...
    categories, args.train_list, args.val_list, args.root_path, prefix = datasets_video.return_dataset('thumos-fs', args.modality) # load few-shot dataset
    ...
    fs_model.module.consensus.classifier = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Linear(in_features = 512, out_features = num_fs_class,  bias = True)
    ).cuda()
    ...
    args.store_name = '_'.join(['fs_TRN', args.dataset, args.modality, args.arch, args.consensus_type, 'segment%d'% args.num_segments])
    print('storing name: ' + args.store_name)

    best_prec1 = 0
    log_fs_training = open(os.path.join(args.root_log, '%s.csv' % "fs-logging"), 'w')
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, fs_model, criterion, optimizer, epoch, log_fs_training)

        # evaluate on validation set
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            prec1 = validate(val_loader, fs_model, criterion, (epoch + 1) * len(train_loader), log_fs_training)
            ...
            save_checkpoint({
                ...
                'best_prec1': best_prec1,
            }, is_best, filename = 'fs_checkpoint.pth.tar')
